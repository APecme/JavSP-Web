...
- 在“系统设置”添加 qBittorrent 下载器，测试连接后可在“下载管理”设置接管、做种和下载完成自动刮削规则。连接由 JavSP WEB 服务端发起：Docker 中的 `127.0.0.1` 是 JavSP WEB 容器本身；同一 Docker 网络请填写 qBittorrent 服务名，例如 `http://qbittorrent:8080`；qBittorrent 在 Docker 宿主机时可填写 `http://host.docker.internal:端口`；跨服务器时填写服务端可访问的域名或 IP。
